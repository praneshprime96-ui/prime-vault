...
// Automatic Pricing Function based on Coin Name
function calculateAutomaticPrice(coinName) {
    const nameLower = coinName.toLowerCase();
    
    // Check for specific keywords or set default estimated values
    if (nameLower.includes("brihadeeswarar") || nameLower.includes("1000 years")) {
        return "₹350.00";
    } else if (nameLower.includes("5 rupees") || nameLower.includes("5 rupee")) {
        return "₹135.75";
    } else if (nameLower.includes("10 rupees") || nameLower.includes("10 rupee")) {
        return "₹220.50";
    } else if (nameLower.includes("1 rupee")) {
        return "₹75.00";
    } else {
        return "₹100.00"; // Default baseline price
    }
}

// Add Coin Handler (Price is now generated automatically!)
addCoinBtn.addEventListener("click", async () => {
    ...
    // Automatically calculate the price based on the name given
    const autoPrice = calculateAutomaticPrice(coinName);
    ...
    reader.onload = async () => {
        ...
        try {
            await db.collection("coins").add({
                name: coinName,
                year: coinYear,
                price: autoPrice, // Saved automatically
                image: base64Image,
...
function loadCoins() {
    ...
    db.collection("coins").orderBy("createdAt", "desc").get().then((snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            let deleteBtnHTML = "";
            if (currentUser && coin.userId === currentUser.uid) {
            ...
            card.innerHTML = `
                ...
                <div class="coin-price">${coin.price || '₹100.00'}</div>
